...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	$info = getIPAPI($ip);
	if(checkBlockedUser($username) || checkBlockedIP() || checkBlockedCIDR())
	{
	...
	if($count == 0)
		$last_id = $db->insert('users', $users_array,);
	else
	{
		$last_id = $db->where('ip_addr',$ip)->update('users', $users_array,);
	}
	...
	if ($db->count >= 1) {
		...
		$_SESSION['login_failure'] = "No users";
		header('Location:login.php');
		exit;
	}
